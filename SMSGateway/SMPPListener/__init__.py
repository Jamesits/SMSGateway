...
def smpp_message_send_handler(pdu):
    logger.info(f"sent {pdu.sequence} {pdu.message_id}")


def smpp_message_receive_handler(pdu):
    logger.info(f"delivered {pdu.receipted_message_id}")
    if pdu.command == "deliver_sm":
        # got new message
        logger.info(
            f"New SMS from {pdu.source_addr.decode('ascii', errors='ignore')}: {pdu.short_message.decode('utf-16-be', errors='ignore')}")
